    ...
    def run(self, event):
        ...
        if stbFreq1!=self.__oldStbFreq1:
            if (self.__editCur=="COM1"):
                ...
                self.mqttpublish(Config.topic_display, "T,38,94,0,1,Com1 Stb Freq/Psh SW>")
            else:
        ...
        if stbFreq2!=self.__oldStbFreq2:
            if (self.__editCur=="COM2"):
                ...
                self.mqttpublish(Config.topic_display, "T,38,94,0,1,Com2 Stb Freq/Psh SW>")
            else:
        ...
        if adfFreq!=self.__oldAdfFreq:
            if (self.__editCur=="ADF"):
                ...
                self.mqttpublish(Config.topic_display, "T,38,94,0,1,           ADF Freq>")
            else:
        ...
        if xpdrCode!=self.__oldXpdrCode:
            if (self.__editCur=="XPDR"):
                self.mqttpublish(Config.topic_display, "T,37,98,0,3,"+str(oct(xpdrCode))[2:])
                self.mqttpublish(Config.topic_display, "T,38,94,0,1,       XPDR Code>")
            else:
                self.mqttpublish(Config.topic_display, "T,37,99,1,3,"+str(oct(xpdrCode))[2:])
        ...
        if (not(event.isEmpty())):
            ...
            if event.getTopic()==Config.topic_keys:
                ...
                if event.getPayload()==Const.ENC1DN: # INC1 DN
                    ...
                    if (self.__editCur=="XPDR"):
                        pyuipc.write([(self.__OFFSETS[6][0],self.__OFFSETS[6][1],xpdr_int2bcd(int(str(oct(xpdrCode-1))[2:])))])
                ...
                if event.getPayload()==Const.BTENC: # SW
                    if (self.__editCur=="COM1"):
                ...
                if event.getPayload()==Const.ENC2DN: # INC2 DN
                    ...
                    if (self.__editCur=="XPDR"):
                        pyuipc.write([(self.__OFFSETS[6][0],self.__OFFSETS[6][1],xpdr_int2bcd(int(str(oct(xpdrCode-64))[2:])))])
                    event.clear()
                ...
                if event.getPayload()==Const.RSK4: #COM2
                    self.__editCur="COM2"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.LSK4: #COM1
                    self.__editCur="COM1"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.LSK7: #ADF
                    self.__editCur="ADF"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.RSK7: #XPDR
                    self.__editCur="XPDR"
                    self.draw()
                    event.clear()
