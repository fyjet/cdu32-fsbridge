    ...
    def run(self, event):
        ...
        if (not(event.isEmpty())):
            ...
            if event.getTopic()==Config.topic_keys:
                if event.getPayload()==Const.LSK2: # com1
                    pyuipc.write([(self.__OFFSETS[0][0],self.__OFFSETS[0][1],results[0]^192)])
                    event.clear()

                if event.getPayload()==Const.LCK2: # com2
                    pyuipc.write([(self.__OFFSETS[0][0],self.__OFFSETS[0][1],results[0]^192)])
                    event.clear()

                if event.getPayload()==Const.RCK2: # both
