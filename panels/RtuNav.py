    ...
    def run(self, event):
        ...
        if stbFreq1!=self.__oldStbFreq1:
            if (self.__editCur=="NAV1"):
                ...
                self.mqttpublish(Config.topic_display, "T,38,94,0,1,Nav1 Stb Freq/Psh SW>")
            else:
        ...
        if stbFreq2!=self.__oldStbFreq2:
            if (self.__editCur=="NAV2"):
                ...
                self.mqttpublish(Config.topic_display, "T,38,94,0,1,Nav2 Stb Freq/Psh SW>")
            else:
        ...
        if HdgNav1!=self.__oldHdgNav1:
            if (self.__editCur=="HDG1"):
                ...
                self.mqttpublish(Config.topic_display, "T,38,94,0,1,           OBS1 Heading>")
            else:
        ...
        if HdgNav2!=self.__oldHdgNav2:
            if (self.__editCur=="HDG2"):
                ...
                self.mqttpublish(Config.topic_display, "T,38,94,0,1,           OBS2 Heading>")
            else:
        ...
        if (not(event.isEmpty())):
            ...
            if event.getTopic()==Config.topic_keys:
                ...
                if event.getPayload()==Const.RSK4: #NAV2
                    self.__editCur="NAV2"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.LSK4: #NAV1
                    self.__editCur="NAV1"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.LSK7: #HDG1
                    self.__editCur="HDG1"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.RSK7: #HDG2
                    self.__editCur="HDG2"
                    self.draw()
                    event.clear()
