    ...
    def run(self,event):
        ...
        if (not(event.isEmpty())):
            ...
            if event.getTopic()==Config.topic_keys:
                ...
                if event.getPayload()==Const.ENC1DN: # INC1 DN
                    if (self.__editCur=="ALT"):
                        pyuipc.write([(self.__OFFSETS[0][0],self.__OFFSETS[0][1],int((altitude-99)*19975))])
                    if (self.__editCur=="HDG"):
                        pyuipc.write([(self.__OFFSETS[1][0],self.__OFFSETS[1][1],int(bearing(heading-1)*182))])
                    if (self.__editCur=="VERTSPEED"):
                        pyuipc.write([(self.__OFFSETS[2][0],self.__OFFSETS[2][1],int(vertSpeed-100))])
                    if (self.__editCur=="COURSE"):
                        pyuipc.write([(self.__OFFSETS[3][0],self.__OFFSETS[3][1],int(bearing(Course-1)))])
                    event.clear()

                if event.getPayload()==Const.ENC1UP: # INC1 UP
                    if (self.__editCur=="ALT"):
                        pyuipc.write([(self.__OFFSETS[0][0],self.__OFFSETS[0][1],int((altitude+101)*19975))])
                    if (self.__editCur=="HDG"):
                        pyuipc.write([(self.__OFFSETS[1][0],self.__OFFSETS[1][1],int(bearing(heading+1)*182))])
                    if (self.__editCur=="VERTSPEED"):
                        pyuipc.write([(self.__OFFSETS[2][0],self.__OFFSETS[2][1],int(vertSpeed+100))])
                ...
                if event.getPayload()==Const.ENC2DN: # INC2 DN
                    if (self.__editCur=="ALT"):
                        pyuipc.write([(self.__OFFSETS[0][0],self.__OFFSETS[0][1],int((altitude-999)*19975))])
                    if (self.__editCur=="HDG"):
                        pyuipc.write([(self.__OFFSETS[1][0],self.__OFFSETS[1][1],int(bearing(heading-10)*182))])
                    if (self.__editCur=="VERTSPEED"):
                        pyuipc.write([(self.__OFFSETS[2][0],self.__OFFSETS[2][1],int(vertSpeed-100))])
                    if (self.__editCur=="COURSE"):
                        pyuipc.write([(self.__OFFSETS[3][0],self.__OFFSETS[3][1],int(bearing(Course-10)))])
                    event.clear()

                if event.getPayload()==Const.ENC2UP: # INC2 UP
                    if (self.__editCur=="ALT"):
                        pyuipc.write([(self.__OFFSETS[0][0],self.__OFFSETS[0][1],int((altitude+1001)*19975))])
                    if (self.__editCur=="HDG"):
                        pyuipc.write([(self.__OFFSETS[1][0],self.__OFFSETS[1][1],int(bearing(heading+10)*182))])
                    if (self.__editCur=="VERTSPEED"):
                        pyuipc.write([(self.__OFFSETS[2][0],self.__OFFSETS[2][1],int(vertSpeed+100))])
                ...
                if event.getPayload()==Const.LSK3: #ALT
                    self.__editCur="ALT"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.LSK4: # LockAlt
                    lockALT=lockALT^1
                    pyuipc.write([(self.__OFFSETS[5][0],self.__OFFSETS[5][1],lockALT)])
                    event.clear()

                if event.getPayload()==Const.LCK3: #IAS
                    self.__editCur="IAS"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.LCK4: # LockIAS
                    lockIAS=lockIAS^1
                    pyuipc.write([(self.__OFFSETS[6][0],self.__OFFSETS[6][1],lockIAS)])
                    event.clear()

                if event.getPayload()==Const.LCK5: # LockAT
                    lockAT=lockAT^1
                    pyuipc.write([(self.__OFFSETS[8][0],self.__OFFSETS[8][1],lockAT)])
                    event.clear()

                if event.getPayload()==Const.RCK3: #HDG
                    self.__editCur="HDG"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.RCK4: # LockHDG
                    lockHDG=lockHDG^1
                    pyuipc.write([(self.__OFFSETS[7][0],self.__OFFSETS[7][1],lockHDG)])
                    event.clear()

                if event.getPayload()==Const.LSK6: #VERTSPEED
                    self.__editCur="VERTSPEED"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.RSK3: #COURSE
                    self.__editCur="COURSE"
                    self.draw()
                    event.clear()

                if event.getPayload()==Const.RSK4: # LockNAV
                    lockNAV=lockNAV^1
                    pyuipc.write([(self.__OFFSETS[9][0],self.__OFFSETS[9][1],lockNAV)])
                    event.clear()

                if event.getPayload()==Const.RSK5: # LockBC
                    lockBC=lockBC^1
                    pyuipc.write([(self.__OFFSETS[10][0],self.__OFFSETS[10][1],lockBC)])
                    event.clear()

                if event.getPayload()==Const.RSK6: # LockAPP
                    lockAPP=lockAPP^1
                ...
                if event.getPayload()==Const.LCK7: # LockATT
                    lockATT=lockATT^1
                    pyuipc.write([(self.__OFFSETS[12][0],self.__OFFSETS[12][1],lockATT)])
                    event.clear()

                if event.getPayload()==Const.RCK7: # LockYD
                    lockYD=lockYD^1
