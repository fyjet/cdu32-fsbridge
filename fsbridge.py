...
def on_message(client, userdata, msg):
    ...
    logging.debug("Event sent: topic="+event.getTopic()+" payload="+str(event.getPayload())+" isempty="+str(event.isEmpty()))
...
def loop():
    ...
    panel.run(event)
    if event.getPayload()==Const.BTMAIN: #return to home
        actMode="Home"
        event.clear()
    if event.getPayload()==Const.BTAP: # AP
        APStatus=APStatus^1
        event.clear()
    ...
    if ((time.time() - alivetime) > 2):
        if (config.get('ESP32','keepalive') == 'YES'):
